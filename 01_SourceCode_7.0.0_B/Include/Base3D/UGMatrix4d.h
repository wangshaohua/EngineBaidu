...
public:
	...
	UGbool operator == (const UGMatrix4d &m2);
	...
	UGdouble MINOR(const UGMatrix4d& m, const UGint r0, const UGint r1, const UGint r2, 
		const UGint c0, const UGint c1, const UGint c2);

	//! \brief 重载赋值运算符。
	void operator = ( const UGMatrix3d& mat3 );
	
	//! \brief 设置矩阵平移部分，[0][3]，[2][3]，[2][3]的值。
	//! \param v 平移向量[in]。
	void SetTrans( const UGVector3d& v);	
	
	//! \brief 获取矩阵平移部分，[0][3]，[2][3]，[2][3]的值。
	//! \param v 平移向量[in]。
	UGVector3d GetTrans();
